    ...
    public class GarmentReceiptCorrectionReportFacade:IGarmentReceiptCorrectionReportFacade
    {
        ...
        public IQueryable<GarmentReceiptCorrectionReportViewModel> GetQuery(string unit, string category, DateTimeOffset? dateFrom, DateTimeOffset? dateTo, string order)
        {
            ...
            var Query = (from a in dbContext.GarmentReceiptCorrections
                        ...
                        && (category == "Bahan Baku" ? Status.Contains(h.ProductName) : (category == "Bahan Pendukung" ? Status.Contains(h.ProductName) : (category == "Bahan Embalace" ? Status.Contains(h.ProductName) : h.ProductName == h.ProductName)))
                         orderby
        ...
        public MemoryStream GenerateExcel(string unit, string category, DateTimeOffset? dateFrom, DateTimeOffset? dateTo, string order)
        {
            ...
            else
            {
                ...
                foreach (var item in Query)
                {
                    ...
                    result.Rows.Add(index, item.CorrectionNo, GetFormattedDate(item.CorrectionDate), item.CorrectionNote, item.URNNo, item.BillNo, 
